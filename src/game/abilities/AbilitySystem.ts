...
export class AbilitySystem {
  ...
  private useBolt(time: number, cooldownMultiplier: number): void {
    ...
    for (let i = 0; i < count; i += 1) {
      ...
      this.host.firePlayerProjectile({
        ...
        scale: level >= 8 ? 1.25 : 0.9, tint: level >= 8 ? 0xfff1a6 : 0x72e6d2, ability: 'bolt',
      });
    }
  ...
  private updateOrbs(time: number, delta: number): void {
    ...
    const radius = 58 + Math.floor(level / 2) * 7;
    for (let i = 0; i < this.orbs.length; i += 1) {
      const angle = this.orbAngle + (Math.PI * 2 * i) / this.orbs.length;
      const orb = this.orbs[i];
      orb.setPosition(this.host.player.x + Math.cos(angle) * radius, this.host.player.y + Math.sin(angle) * radius);
      orb.setDepth(this.host.player.y + Math.sin(angle) * radius + 3).setScale(level >= 8 ? 1.25 : 0.84 + level * 0.03);
      orb.angle += 3.5;
  ...
  private createMeteor(x: number, y: number, level: number, extraDelay: number): void {
    const ring = this.scene.add.circle(x, y, 45 + level * 4, 0xff6b3c, 0.13).setStrokeStyle(3, 0xffb357, 0.9).setDepth(6800);
    this.scene.tweens.add({ targets: ring, scale: 0.55, alpha: 0.55, duration: 760 + extraDelay, onComplete: () => {
      const meteor = this.scene.add.image(x - 110, y - 180, 'meteor').setScale(1.25 + level * 0.05).setDepth(9000);
      this.scene.tweens.add({ targets: meteor, x, y, angle: 180, duration: 260, ease: 'Quad.in', onComplete: () => {
        meteor.destroy();
        ring.destroy();
        this.host.areaDamage(x, y, 70 + level * 7, 42 + level * 18, 'meteor', { tint: 0xff8a4d });
        this.host.burst(x, y, 0xff8a4d, 24, 220);
  ...
  private usePoison(time: number, cooldownMultiplier: number): void {
    ...
    this.scene.tweens.add({
      ...
      onComplete: () => {
        flask.destroy();
        this.host.createPoisonPool(target.x, target.y, 55 + level * 6, 3800 + level * 380, 8 + level * 5);
      },
    });
  ...
  private useShuriken(time: number, cooldownMultiplier: number): void {
    ...
    for (let i = 0; i < count; i += 1) {
      this.host.firePlayerProjectile({
        x: this.host.player.x, y: this.host.player.y, angle: base + (i - (count - 1) / 2) * 0.19,
        texture: 'projectile-shuriken', speed: 430 + level * 20, damage: 18 + level * 9,
        lifespan: 2100, pierce: 1 + Math.floor(level / 2), scale: 0.8 + level * 0.035,
  ...
  private useLaser(time: number, cooldownMultiplier: number): void {
    ...
    const length = 720;
    const end = new Phaser.Math.Vector2(origin.x + Math.cos(angle) * length, origin.y + Math.sin(angle) * length);
    const graphics = this.scene.add.graphics().setDepth(9100);
    graphics.lineStyle(10 + level * 1.2, 0xfff2a2, 0.16).lineBetween(origin.x, origin.y, end.x, end.y);
    graphics.lineStyle(3 + level * 0.45, 0xd8ffff, 0.92).lineBetween(origin.x, origin.y, end.x, end.y);
    ...
    for (const foe of this.host.getActiveFoes()) {
      ...
      if (Phaser.Math.Distance.Between(closest.x, closest.y, foe.x, foe.y) < 14 + level * 2) {
        this.host.dealDamage(foe, 32 + level * 13, 'laser', { tint: 0xfff2a2 });
      }
    }
  ...
  private useArrow(time: number, cooldownMultiplier: number): void {
    ...
    for (let i = 0; i < count; i += 1) {
      ...
      this.host.firePlayerProjectile({
        ...
        speed: 330 + level * 18, damage: 20 + level * 10, lifespan: 6500 + level * 900,
        pierce: 1 + Math.floor(level / 3), scale: 0.86 + level * 0.04, tint: 0x75dfff,
  ...
  private useLightning(time: number, cooldownMultiplier: number): void {
    ...
    for (let i = 0; i < chains && current; i += 1) {
      ...
      graphics.lineStyle(7, 0x6db8ff, 0.16).strokePoints(points);
      graphics.lineStyle(2, 0xdffaff, 1).strokePoints(points);
  ...
  private useFireRing(time: number, cooldownMultiplier: number): void {
    ...
    const radius = 92 + level * 11;
    const ring = this.scene.add.circle(this.host.player.x, this.host.player.y, 18, 0xff6f3d, 0).setStrokeStyle(9, 0xff8a4f, 0.8).setDepth(8800);
  ...
  private useIceStorm(time: number, cooldownMultiplier: number): void {
    ...
    for (const foe of foes.slice(0, 2 + Math.floor(level / 2))) {
      const shard = this.scene.add.image(foe.x, foe.y - 90, 'ice-shard').setDepth(9000).setScale(0.9);
      this.scene.tweens.add({ targets: shard, y: foe.y, angle: 180, duration: 320, onComplete: () => {
        shard.destroy();
        this.host.areaDamage(foe.x, foe.y, 35 + level * 3, 18 + level * 8, 'iceStorm', { slow: 0.55, slowDuration: 1900 + level * 180, tint: 0xa8efff });
      }});
    }
  ...
  private useBlackHole(time: number, cooldownMultiplier: number): void {
    ...
    const target = this.host.findDenseFoe();
    if (!target) return;
    this.host.createBlackHole(target.x, target.y, 95 + level * 7, 2600 + level * 220, 8 + level * 5);
    this.nextUse.blackHole = time + Math.max(6500, (14_000 - level * 650) * cooldownMultiplier);
  }

  private syncOrbs(): void {
    const level = this.getLevel('orb');
    const desired = level >= 8 ? 8 : 1 + Math.floor((level - 1) / 2);
